...
- 対処: 次のサイクルまで待ってください。DeepL Free プランのクォータは月次で自動的にリセットされます。課金は不要で、翌月には自動的に再び利用可能になります。
- ヒント: **Limits** の **Max characters per request** を下げると、大きなページでの消費を抑えやすくなります。
- 確認方法: 現在の使用量と次のリセット日は [deepl.com/account/usage](https://www.deepl.com/account/usage) で確認できます。
- 注記: DeepL API Pro へのアップグレードは任意であり、必須ではありません。
